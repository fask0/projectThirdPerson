...
void WobbleMaterial::_lazyInitializeShader()
{
	if (!_shader)
	{
		...
		_shader->addShader(GL_VERTEX_SHADER, config::MGE_SHADER_PATH + "wobbleTexture.vs");
		_shader->addShader(GL_FRAGMENT_SHADER, config::MGE_SHADER_PATH + "wobbleTexture.fs");
		_shader->finalize();
...
void WobbleMaterial::render(World* pWorld, Mesh* pMesh, const glm::mat4& pModelMatrix, const glm::mat4& pViewMatrix, const glm::mat4& pProjectionMatrix)
{
	...
	glUniform1f(_uTime, clock() / 1000.0f);
